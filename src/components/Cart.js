...
export default (props) => {
    const { cartStatus, closeCart, openCart, checkoutState, setCount } = useShopify();
        ...
        <div id="cart">
            <div className={`Cart ${cartStatus ? 'Cart--open' : ''}`}>
                <div className="App__view-cart-wrapper2">
                    <button
                        className={`App__view-cart ${cartStatus ? 'hide' : ''}`}
                        onClick={(e) => handleOpen(e)}
                    >
                        <img src="../images/cart.svg" width="30" height="30" alt="cart" />
                    </button>
                </div>
